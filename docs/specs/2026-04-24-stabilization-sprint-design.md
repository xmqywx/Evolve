...
| # | 文件 | 动作 | 内容 | 粗估 |
|---|------|------|------|------|
| 1 | `docs/ARCHITECTURE.md` | **新** | 一页纸总图 + 词汇表 + 进程拓扑图 + 数据流图。≤400 行，图表为主 | 0.5 天 |
| 2a | `docs/specs/SPEC_LEDGER.md` | **新** | 5 份老 spec 索引：`Active / Partial / Superseded-by / Deprecated`，每份一行。暂用文件名引用（无 § 号） | 0.2 天 |
| 3 | `docs/OVERVIEW.md` | **新** | V2 主干合并版：产品定位 + **Self-Report 总线（含 bus 身份定位 + 未来多数字人接入说明）** + 三页新 UI + 生存引擎升级。吸收 v2-design + workflow-v2 + knowledge-hub + chat-survival-system 的有用部分。≤600 行 | 0.5 天 |
| 2b | 老 spec 顶部横幅 | **改** | 5 份老 spec 顶部加 `> Status: <状态> — Superseded by docs/OVERVIEW.md § X`，§ 号引用 step 3 已写好的实际章节号 | 0.2 天 |
...
| 6 | `docs/decisions/2026-04-24-roles-boundary.md` | **新** | ADR：persona=人格层 / agents=技能卡层 / digital_humans=未来运行时层。含 `persona/knowledge.md` `persona/principles.md` 的处置：**填最小版本**（已在 §6.2 草拟） | 0.3 天 |
| 7 | `CLAUDE.md` | **改** | 技术栈段落：tmux→cmux、claude→codex (默认)、Ant Design→Tailwind (迁移中) | 0.1 天 |
| 8 | 全仓 `rg "tmux"` 残留 | **审** | 剩下的全是 "已迁 cmux" 或 "历史遗留" 注释；代码层残留只标注不改 | 0.2 天 |
...
> 注：原 step 9（Self-Report 总线定位段落）已并入 step 3 的 OVERVIEW.md，不单独列。
> 原 step 2 拆成 2a（写 ledger，用文件名引用）和 2b（OVERVIEW 写完后回填 § 号横幅），避免 step 2 citing 未存在的章节号。

| 步 | 产出 | 完成标准 |
|---|---|---|
| **1** | `ARCHITECTURE.md` + 词汇表 | 能用它一页纸给外人解释 MyAgent；词汇表 5 个条目齐全；**含进程拓扑图和数据流图各 1 张**（mermaid 或 ASCII） |
| **2a** | `SPEC_LEDGER.md` | 5 行无遗漏，每行含状态 + 去向（用文件名，不引 § 号） |
| **3** | `OVERVIEW.md` | 不再需要读 5 份老 spec 才能理解 V2；**Self-Report API 章节以 "bus" 身份出现，含未来多数字人接入说明一段**；行数 ≤600 |
| **2b** | 5 份老 spec 顶部横幅 | 每份第一行是 `> Status: <状态> — Superseded by docs/OVERVIEW.md § N.M`，§ 号为 step 3 已成文的真实章节号 |
| **4** | `PROGRESS.md` | Phase 1-5 每条有明确 ✅/🚧/⬜ 和证据（commit hash / 文件路径） |
| **5** | storage ADR | 写出"为什么 SQLite 赢"+ Postgres 下线动作清单（未执行）；`config.yaml` **本轮不动** |
| **6** | roles-boundary ADR + persona 文件回填 | ADR 定稿；`persona/knowledge.md` 和 `persona/principles.md` 已填最小版本（3-5 行），无 TODO |
| **7** | `CLAUDE.md` 技术栈段 | 字符串与现实一致（cmux, codex, tailwind-migrating） |
| **8** | `rg "tmux"` 审计 | 残留全部有说明性上下文（迁移注释/历史引用）；代码层残留只在文档标注，不动代码 |
...
| **步骤 8 tmux 残留超 20 处（文档+代码）超出 0.2 天预算** | 分两类：文档层逐条补 "已迁 cmux" 注释；代码层只列清单写进 `PROGRESS.md` 留下一轮，不展开。超预算立即停手 |
| **步骤 3 OVERVIEW.md 吸收 4 份 spec 内容撑破 600 行** | 硬砍：把 workflow-v2 / knowledge-hub 的纯实现细节剥离成"OVERVIEW § X 引用原 spec 作为实施细节来源"，OVERVIEW 只保留定位和接口 |
